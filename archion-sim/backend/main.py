...
_sim_config_lock = threading.Lock()
_sim_config: dict | None = None
...
class RoleConfig(BaseModel):
    ...
    count: int
    color: str
    areas: list[list[float]] = Field(default_factory=list)

class SimConfig(BaseModel):
    roles: list[RoleConfig] = Field(default_factory=list)

@app.post("/api/simulation/configure")
async def configure_simulation(config: SimConfig):
    global _sim_config
    with _sim_config_lock:
        _sim_config = config.model_dump()
    return {"status": "ok"}
